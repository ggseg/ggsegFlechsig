...
for(hemi in c("lh", "rh")){
  tmp <- here::here(sprintf("data-raw/flechsig/%s.fsaverage_Flechsig", hemi))
...
# bash
# sudo cp data-raw/flechsig/lh.fsaverage_flechsig.annot $FREESURFER_HOME/subjects/fsaverage/label/lh.flechsig.annot
# sudo cp data-raw/flechsig/rh.fsaverage_flechsig.annot $FREESURFER_HOME/subjects/fsaverage/label/rh.flechsig.annot
...
# Make  3d ----
flechsig_3d <- make_aparc_2_3datlas(
  annot = annot_name,
...
flechsig_n <- flechsig_3d
flechsig_n <- unnest(flechsig_n, ggseg_3d)
flechsig_n <- mutate(flechsig_n,
                    region = ifelse(grepl("Unknown|\\?", region, ignore.case = TRUE),
                                    NA, region),
                    atlas = "flechsig_3d"
)
flechsig_3d <- as_ggseg3d_atlas(flechsig_n)
ggseg3d(atlas  = flechsig_3d)
ggseg3d(atlas  = flechsig_3d, hemisphere = "left")


# Make palette ----
brain_pals <- make_palette_ggseg(flechsig_3d)
usethis::use_data(brain_pals, internal = TRUE, overwrite = TRUE)
devtools::load_all(".")


# Make 2d polygon ----
flechsig <- make_ggseg3d_2_ggseg(flechsig_3d,
                                 ncores = 15,
                                 steps = 6:7,
                                 tolerance = .1,
                                 output_dir = here::here("data-raw/"))
...
flechsig %>%
  ...
  scale_fill_brain("flechsig", package = "ggsegFlechsig", na.value = "black")


usethis::use_data(flechsig, flechsig_3d,
                  internal = FALSE,
                  overwrite = TRUE,
                  compress="xz")
